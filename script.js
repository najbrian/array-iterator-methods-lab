...
inventors.filter((inventor) => {
  inventor.year >= 1500 && inventor.year< 1600 ? veryOldInventors.push(inventor) : ''
})
...
firstLast = people.map(person => {
  const [last, first] = person.split(', '); //split creates new array, and can be categorized into new variable with it's own property and value
  return `${first} ${last}`;
});
...
// Enter your solution code here:
travelMethodCounts = travelMethods.reduce((acc, travel) => {
  acc[travel] ? acc[travel] ++ : acc[travel]=1
  return acc;
}, {})
